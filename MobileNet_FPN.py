...
class FPN_MobileNetV3(nn.Module):
    def __init__(self, num_classes=2):
        super(FPN_MobileNetV3, self).__init__()

        backbone = models.mobilenet_v3_large(weights="IMAGENET1K_V1").features
        
        self.layer1 = backbone[:3]   # C2 (16)
        self.layer2 = backbone[3:6]  # C3 (40)
        self.layer3 = backbone[6:12] # C4 (112)
        self.layer4 = backbone[12:]  # C5 (960)

        self.toplayer = nn.Conv2d(960, 256, kernel_size=1)  # P5
        self.latlayer1 = nn.Conv2d(112, 256, kernel_size=1) # C4
        self.latlayer2 = nn.Conv2d(40, 256, kernel_size=1)  # C3
        self.latlayer3 = nn.Conv2d(24, 256, kernel_size=1)  # C2

        self.c5_reduce = nn.Conv2d(960, 256, kernel_size=1)
    ...
    def forward(self, x):
        c2 = self.layer1(x)  # [B,16,112,112]
        c3 = self.layer2(c2) # [B,40,56,56]
        c4 = self.layer3(c3) # [B,112,28,28]
        c5 = self.layer4(c4) # [B,960,14,14]

        p5 = self.toplayer(c5)                       # [B,256,14,14]
        p4 = self._upsample_add(p5, self.latlayer1(c4)) # [B,256,28,28]
        p3 = self._upsample_add(p4, self.latlayer2(c3)) # [B,256,56,56]
        p2 = self._upsample_add(p3, self.latlayer3(c2)) # [B,256,112,112]

        p4 = self.smooth1(p4)
        p3 = self.smooth2(p3)
        p2 = self.smooth3(p2)

        c5_reduced = self.c5_reduce(c5)                # [B,256,14,14]
        ...
        c5_pool = self.global_avg_pool(c5_reduced).flatten(1) # [B,256]

        features = torch.cat([p2_pool, p3_pool, p4_pool, p5_pool, c5_pool], dim=1) # [B,1280]

        out = self.classifier(features)
        return out

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = FPN_MobileNetV3().to(device)
